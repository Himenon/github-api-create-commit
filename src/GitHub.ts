...
export class GitHub {
	...
	public createGitCommit = async ({
		...
		const parentCommit = await this.getParentCommit({
			headBranchName,
			baseBranchName,
		});
	...
	private createBranch = async ({
		branchName,
		baseBranchName,
	}: Types.CreateBranchArgs): Promise<void> => {
		const owner = this.owner;
		const repo = this.repo;
		let baseRef: string;
		if (baseBranchName) {
	 ...
	 */
	private getParentCommit = async ({
		headBranchName,
		baseBranchName,
	}: Types.GetParentCommitArgs): Promise<Schemas.commit> => {
		const owner = this.owner;
		const repo = this.repo;
		...
		if (!branches.find((b) => b.name === headBranchName)) {
			const targetBaseBranchName =
				baseBranchName && branches.find((b) => b.name === baseBranchName)
					? baseBranchName
					: undefined;
			await this.createBranch({
				branchName: headBranchName,
				baseBranchName: targetBaseBranchName,
			});
		}
