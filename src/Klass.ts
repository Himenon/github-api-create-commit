...
export class GithubCreateCommitMixin {
  constructor(private readonly client: Client<any>, private readonly owner: string, private readonly repo: string) {}
  private async createBranch({ branchName, baseBranchName }: Types.CreateBranchArgs): Promise<void> {
   ...
   */
  private async getParentCommit({ headBranchName, baseBranchName }: Types.GetParentCommit) {
    const owner = this.owner;
    const repo = this.repo;
